...
class Review:
    ...
    def employee_id(self, employee_id):
        # Check if the provided employee_id corresponds to an existing employee
        if isinstance(employee_id, int) and Employee.find_by_id(employee_id):
            self._employee_id = employee_id
        else:
            raise ValueError("Employee ID should be an integer that references an existing employee")
    ...
    def find_by_id(cls, id):
        """Return a Review instance corresponding to the row in the "reviews" table with that same id, or None if no such row exists in the table."""
        sql = """
            SELECT *
            FROM reviews
            WHERE id = ?
